...
const filtered = computed(() => {
  ...
  if (s) list = list.filter(i => (i.title || '').toLowerCase().includes(s) || (i.id || '').toLowerCase().includes(s) || (i.symptom || '').toLowerCase().includes(s))
  return list
})
...
function handleAction(action, id) {
  if (action === 'view') router.push('/fault/detail/' + id)
  else if (action === 'edit') router.push('/fault/edit/' + id)
  else if (action === 'delete') {
    modal.value = { visible: true, message: '确定要删除这条故障吗？', type: 'confirm', action: 'delete-single', payload: id }
  }
}

async function doDelete(id) {
  const result = await submitWithApproval('fault', 'DELETE', null, id, () => apiFaults.delete(id))
...
async function doBatchDelete() {
  try {
    await apiFaults.batchDelete([...selected.value])
    selected.value = []
    await loadData()
...
async function loadData() {
  try { const res = await apiFaults.list(); items.value = res.data }
