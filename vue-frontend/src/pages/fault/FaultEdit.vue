  ...
  <MainLayout>
    <div class="max-w-[720px] mx-auto">
      ...
      <div v-else class="form-card">
        ...
        <div class="form-group">
          <label>故障现象</label>
          <textarea v-model="form.symptom" rows="3" class="form-input"></textarea>
        </div>

        <div class="form-group">
          <label>故障原因</label>
          <textarea v-model="form.cause" rows="3" class="form-input"></textarea>
        </div>

        <div class="form-group">
          <label>解决方案</label>
          <textarea v-model="form.solution" rows="5" class="form-input"></textarea>
        </div>
...
const form = ref({ id: '', title: '', category: '', symptom: '', cause: '', solution: '' })
...
onMounted(async () => {
  try {
    ...
    form.value.symptom = d.symptom || ''
    form.value.cause = d.cause || ''
    form.value.solution = d.solution || ''
    const imgs = (d.images || []).map(url => ({ url, type: 'image' }))
    const vids = (d.videos || []).map(url => ({ url, type: 'video' }))
    mediaItems.value = [...imgs, ...vids]
