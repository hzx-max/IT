  ...
  <MainLayout>
    ...
    <div class="cmd-grid" :class="{ 'grid-4': sidebarCollapsed }">
      <div v-for="item in filtered" :key="item.id" class="cmd-card" @click="handleClick(item)">
        <FavoriteButton class="card-favorite" module="ai" :item="item" />
...
.cmd-card:hover{overflow:visible;z-index:20;border-color:#7c3aed;box-shadow:0 8px 25px rgba(124,58,237,.12);transform:translateY(-3px)}
...
.scenario-text{font-size:17px;color:#64748b;line-height:1.5;margin:0;text-indent:2em}
.tag-row{display:flex;align-items:center;gap:6px;flex-wrap:nowrap;overflow:hidden;min-width:0;width:100%;max-width:100%}
.tag-row .tag-cat{font-size:16px;padding:3px 10px;border-radius:12px;font-weight:500;background:#f5f3ff;color:#7c3aed;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;flex-shrink:1;min-width:0}
