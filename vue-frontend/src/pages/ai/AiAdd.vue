    ...
    <div class="max-w-[720px] mx-auto">
      ...
      <div class="form-card">
        <div class="form-group">
          ...
          <input v-model="form.title" placeholder="输入AI运维标题" class="form-input">
        </div>
        ...
        <div class="form-group">
          <label>AI场景</label>
          <textarea v-model="form.scenario" rows="3" placeholder="描述AI应用场景" class="form-input"></textarea>
        </div>
        ...
        <div class="form-group">
          ...
          <div v-if="uploading" class="mt-2 text-sm text-purple-600">上传中...</div>
        </div>

        <div class="form-group">
          <label>提示词</label>
          <textarea v-model="form.prompt" rows="5" placeholder="输入提示词/Prompt" class="form-input font-mono"></textarea>
        </div>

        <div class="form-group">
          <label>配置</label>
          <textarea v-model="form.config" rows="5" placeholder="AI工具配置参数" class="form-input font-mono"></textarea>
        </div>

        <div class="form-group">
          <label>描述</label>
          <textarea v-model="form.desc" rows="3" placeholder="简要描述" class="form-input"></textarea>
        </div>
...
const form = ref({ title: '', category: '', scenario: '', prompt: '', config: '', desc: '', detail: '' })
...
function generateId() {
  return 'ai_' + Date.now() + '_' + Math.random().toString(36).substring(2, 6)
}
...
async function onSubmit() {
  ...
  try {
    ...
    const result = await submitWithApproval('ai', 'CREATE', dto, null, () => apiAi.create(dto))
    if (result.ok) {
...
function onModalConfirm() { modal.value.visible = false; if (modal.value.action === 'ok') router.push('/ai') }
...
.btn-primary{background:#7c3aed;color:#fff;box-shadow:0 1px 3px rgba(124,58,237,.3)}
.btn-primary:hover{background:#6d28d9;box-shadow:0 4px 14px rgba(124,58,237,.35);transform:translateY(-1px)}
