    ...
    <div class="max-w-[720px] mx-auto">
      ...
      <div v-else class="form-card">
        ...
        <div class="form-group">
          ...
          <ComboBox v-model="form.category" :options="catOptions" placeholder="选择或输入分类" />
        </div>

        <div class="form-group">
          <label>AI场景</label>
          <textarea v-model="form.scenario" rows="3" class="form-input"></textarea>
        </div>
        ...
        <div class="form-group">
          <label>提示词</label>
          <textarea v-model="form.prompt" rows="5" class="form-input font-mono"></textarea>
        </div>
...
const form = ref({ id: '', title: '', category: '', scenario: '', prompt: '', config: '', desc: '', detail: '' })
const catOptions = computed(() => CATEGORIES.map(v => ({ value: v, label: v })))
...
onMounted(async () => {
  try {
    ...
    form.value.scenario = d.scenario || ''
    form.value.prompt = d.prompt || ''
