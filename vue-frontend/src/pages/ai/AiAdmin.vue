      ...
      <table class="w-full border-collapse text-sm">
        <thead>
          <tr class="bg-slate-50">
            <th class="sticky-th px-4 py-3.5 text-center w-10"><input type="checkbox" :checked="isAllSelected" @change="toggleAll" class="w-[18px] h-[18px] accent-purple-600 cursor-pointer"></th>
        ...
        <tbody>
          ...
          <tr v-for="(item, idx) in filtered" :key="item.id" class="border-t border-slate-100 hover:bg-purple-50 transition-all duration-150" :class="{'bg-purple-50': selected.includes(item.id)}">
            <td class="px-4 py-3 text-center"><input type="checkbox" :checked="selected.includes(item.id)" @change="toggleItem(item.id)" class="w-[18px] h-[18px] accent-purple-600 cursor-pointer"></td>
...
async function doDelete(id) {
  try { await apiAi.delete(id); selected.value = selected.value.filter(i => i !== id); await loadData() }
