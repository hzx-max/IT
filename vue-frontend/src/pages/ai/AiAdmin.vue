<template>
  <MainLayout>
      ...
      <table class="w-full border-collapse text-sm table-fixed">
        ...
        <thead>
          <tr class="bg-slate-50">
            <th class="sticky-th px-2 py-3.5 text-center"><input type="checkbox" :checked="isAllSelected" @change="toggleAll" class="w-[18px] h-[18px] accent-purple-600 cursor-pointer"></th>
        ...
        <tbody>
          <tr v-if="filtered.length === 0"><td colspan="6" class="text-center py-[60px] text-slate-400 text-base">
          ...
          <tr v-for="(item, idx) in filtered" :key="item.id" class="border-t border-slate-100 hover:bg-purple-50 transition-all duration-150" :class="{'bg-purple-50': selected.includes(item.id)}">
            <td class="px-2 py-3 text-center"><input type="checkbox" :checked="selected.includes(item.id)" @change="toggleItem(item.id)" class="w-[18px] h-[18px] accent-purple-600 cursor-pointer"></td>
            <td class="px-2 py-3 text-center text-slate-600">{{ idx + 1 }}</td>
            <td class="px-4 py-3 text-center font-medium">{{ item.title }}</td>
            <td class="px-2 py-3 text-center text-slate-600">{{ item.category }}</td>
    ...
    <div v-if="selected.length > 0" class="fixed bottom-0 left-[240px] right-0 z-[1000] flex items-center gap-3.5 px-5 py-3.5 bg-purple-50 border-t border-purple-200 text-sm font-medium text-purple-600 animate-fadeUp">
...
function handleAction(e, id) {
  ...
  e.target.value = ''
  if (action === 'view') router.push('/ai/detail/' + id)
  else if (action === 'edit') router.push('/ai/edit/' + id)
  else if (action === 'delete') {
    modal.value = { visible: true, message: '确定要删除这条AI运维记录吗？', type: 'confirm', action: 'delete-single', payload: id }
  }
}

async function doDelete(id) {
  const result = await submitWithApproval('ai', 'DELETE', null, id, () => apiAi.delete(id))
...
async function doBatchDelete() {
  try {
    await apiAi.batchDelete([...selected.value])
    selected.value = []
    await loadData()
...
async function loadData() {
  try { const res = await apiAi.list(); items.value = res.data }
...
.top-bar h2::after{content:'';position:absolute;bottom:0;left:0;width:40px;height:3px;background:linear-gradient(90deg,#7c3aed,#a855f7);border-radius:2px}
...
.action-select:hover{border-color:#7c3aed}
.action-select:focus{border-color:#7c3aed;box-shadow:0 0 0 3px rgba(124,58,237,.12)}
...
.btn-ghost-sm:hover{border-color:#7c3aed;color:#7c3aed;background:#f5f3ff}
