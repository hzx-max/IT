  ...
  <MainLayout>
    ...
    <div class="export-pdf-area pl-8 pb-4">
      <div class="detail-actions">
        <button class="detail-action-btn" @click="$router.push('/linux')">← 返回</button>
        <div class="flex gap-2.5">
          <FavoriteButton module="linux" :item="item" show-text v-if="item?.id" />
          <button class="detail-action-btn" @click="$router.push('/linux/edit/'+$route.params.id)" v-if="item?.id">编辑</button>
      ...
      <div v-if="item?.cat" class="detail-section"><div class="detail-label">分类</div><div class="detail-value">{{ getCatLabel(item.cat, LINUX_CAT_MAP) }}</div></div>
    ...
    <RelatedPanel :api-list="apiLinux.list" :current-id="route.params.id" :current-cat="item?.cat" base-path="/linux" />
    </div>
  </MainLayout>
</template>
...
.tag-cat{display:inline-block;padding:4px 12px;border-radius:12px;font-size:14px;font-weight:500;background:#fff7ed;color:#ea580c;border:1.5px solid #fed7aa}
