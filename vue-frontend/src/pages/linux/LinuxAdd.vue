    ...
    <div class="max-w-[720px] mx-auto">
      ...
      <div class="form-card">
        <div class="form-group">
          ...
          <input v-model="form.title" placeholder="输入标题" class="form-input">
        </div>
        ...
        <div class="form-group">
          <label>描述</label>
          <textarea v-model="form.desc" rows="3" placeholder="简要描述该内容" class="form-input"></textarea>
        </div>

        <div class="form-group">
          <label>详细内容</label>
          <textarea v-model="form.detail" rows="6" placeholder="详细的操作说明、使用场景等" class="form-input"></textarea>
        </div>

        <div class="form-group">
          <label>配置操作 <span class="text-red-600">*</span></label>
          <textarea v-model="form.config" rows="5" placeholder="输入配置命令或操作步骤" class="form-input font-mono"></textarea>
        </div>
...
function generateId() {
  return 'linux_' + Date.now() + '_' + Math.random().toString(36).substring(2, 6)
}
...
async function onSubmit() {
  if (!form.value.title || !form.value.vendor || !form.value.cat || !form.value.config) {
    error.value = '请填写标题、发行版、分类和配置操作'
    modal.value = { visible: true, message: '请填写标题、发行版、分类和配置操作', type: 'confirm', action: 'alert' }
    return
  }
  ...
  try {
    ...
    const result = await submitWithApproval('linux', 'CREATE', dto, null, () => apiLinux.create(dto))
    if (result.ok) {
...
function onModalConfirm() { modal.value.visible = false; if (modal.value.action === 'ok') router.push('/linux') }
