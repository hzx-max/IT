<template>
  <MainLayout>
    ...
    <SearchBar v-model="search" placeholder="搜索办公项目..." :showCat="true"
      module="office-admin"
      ...
      <table class="w-full border-collapse text-sm table-fixed">
        ...
        <thead>
          <tr class="bg-slate-50">
            ...
            <th class="sticky-th px-2 py-3.5 text-center text-slate-500 font-semibold text-xs uppercase tracking-wider">厂商</th>
          ...
          <tr v-for="(item, idx) in filtered" :key="item.id" class="border-t border-slate-100 hover:bg-blue-50 transition-all duration-150" :class="{'bg-blue-50': selected.includes(item.id)}">
            ...
            <td class="px-2 py-3 text-center whitespace-nowrap">
              <span v-for="cfg in (item.configs||[]).slice(0, 3)" :key="cfg.vendor"
                ...
                :style="{ background: getVendorColor(cfg.vendor, OFFICE_VENDOR_MAP)+'15', color: getVendorColor(cfg.vendor, OFFICE_VENDOR_MAP), borderColor: getVendorColor(cfg.vendor, OFFICE_VENDOR_MAP)+'40' }">
                {{ getVendorName(cfg.vendor, OFFICE_VENDOR_MAP) }}
              </span>
            </td>
            <td class="px-2 py-3 text-center text-slate-600">{{ item.cat === getCatLabel(item.cat, OFFICE_CAT_MAP) ? item.cat : item.cat + ' - ' + getCatLabel(item.cat, OFFICE_CAT_MAP) }}</td>
...
const dynCatMap = computed(() => {
  const map = {}
  items.value.forEach(i => { if (i.cat && !map[i.cat]) map[i.cat] = getCatLabel(i.cat, OFFICE_CAT_MAP) })
  return map
})
...
function handleAction(action, id) {
  if (action === 'view') router.push('/office/detail/' + id)
  else if (action === 'edit') router.push('/office/edit/' + id)
  else if (action === 'delete') {
    modal.value = { visible: true, message: '确定要删除这项吗？', type: 'confirm', action: 'delete-single', payload: id }
  }
}

async function doDelete(id) {
  const result = await submitWithApproval('office', 'DELETE', null, id, () => apiOffice.delete(id))
...
async function doBatchDelete() {
  try {
    await apiOffice.batchDelete([...selected.value])
    selected.value = []
    await loadData()
...
async function loadData() {
  try { const res = await apiOffice.list(); items.value = res.data }
