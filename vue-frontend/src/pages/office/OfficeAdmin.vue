      ...
      <table class="w-full border-collapse text-sm">
        <thead>
          <tr class="bg-slate-50">
            ...
            <th class="sticky-th px-4 py-3.5 text-center text-slate-500 font-semibold text-xs uppercase tracking-wider">厂商</th>
        ...
        <tbody>
          ...
          <tr v-for="(item, idx) in filtered" :key="item.id" class="border-t border-slate-100 hover:bg-blue-50 transition-all duration-150" :class="{'bg-blue-50': selected.includes(item.id)}">
            ...
            <td class="px-4 py-3 text-center">
              <span v-for="cfg in (item.configs||[])" :key="cfg.vendor"
            ...
            <td class="px-4 py-3 text-center text-slate-600">{{ item.cat === getCatLabel(item.cat, OFFICE_CAT_MAP) ? item.cat : item.cat + ' - ' + getCatLabel(item.cat, OFFICE_CAT_MAP) }}</td>
...
async function doDelete(id) {
  try { await apiOffice.delete(id); selected.value = selected.value.filter(i => i !== id); await loadData() }
