      ...
      <div class="form-card">
        ...
        <div class="form-group">
          ...
          <select v-model="form.vendor" class="form-input">
              <option v-for="(v,k) in OFFICE_VENDOR_MAP" :key="k" :value="k">{{ v.n }}</option>
            </select>
        </div>
        ...
        <div class="form-group">
          <label>描述</label>
          <textarea v-model="form.desc" rows="3" placeholder="简要描述该项目的用途" class="form-input"></textarea>
        </div>
        ...
        <div class="form-group">
          <label>详细内容</label>
          <textarea v-model="form.detail" rows="6" placeholder="详细说明、使用场景等" class="form-input"></textarea>
        </div>
...
const form = ref({ title: '', vendor: 'word', cat: '', desc: '', detail: '', config: '' })

const localCatMap = ref({ ...OFFICE_CAT_MAP })
...
function generateId() {
  return 'office_' + Date.now() + '_' + Math.random().toString(36).substring(2, 6)
}
...
async function onSubmit() {
  if (!form.value.title || !form.value.vendor || !form.value.cat || !form.value.config) {
    error.value = '请填写标题、厂商、分类和配置内容'
    modal.value = { visible: true, message: '请填写标题、厂商、分类和配置内容', type: 'confirm', action: 'alert' }
    return
  }
...
function onModalConfirm() { modal.value.visible = false; if (modal.value.action === 'ok') router.push('/office') }
