  ...
  <MainLayout>
    ...
    <div class="cmd-grid" :class="{ 'grid-4': sidebarCollapsed }">
      <div v-for="item in filtered" :key="item.id" class="cmd-card" @click="handleClick(item)">
        <FavoriteButton class="card-favorite" module="office" :item="item" />
        <div class="cmd-card-body">
          <CardCarousel :images="item.images" :videos="item.videos" />
          <h3>{{ item.title }}</h3>
          <p v-if="item.desc" class="office-desc">{{ truncate(item.desc, 60) }}</p>
...
import { OFFICE_CAT_MAP, getCatLabel, formatDate, apiOffice, apiClicks } from '../../api/index.js'
...
const allCats = computed(() => {
  ...
  return [...keys].sort().map(k => ({ key: k, label: getCatLabel(k, OFFICE_CAT_MAP) }))
})
...
function handleClick(item) {
  recordView('office', item)
  apiClicks.record('office', item.id, item.title).catch(() => {})
  router.push('/office/detail/' + item.id)
}

onMounted(async () => {
  try {
    const res = await apiOffice.list()
...
.office-desc{font-size:17px;color:#64748b;line-height:1.5;margin:0 0 6px;text-indent:2em;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
