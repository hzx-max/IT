    ...
    <div class="max-w-[720px] mx-auto">
      ...
      <div class="form-card">
        <div class="form-group">
          ...
          <input v-model="form.title" placeholder="输入桌面运维问题标题" class="form-input">
        </div>
...
const form = ref({ title: '', category: '', symptom: '', solution: '' })
...
function generateId() {
  return 'desktop_' + Date.now() + '_' + Math.random().toString(36).substring(2, 6)
}
...
function onModalConfirm() { modal.value.visible = false; if (modal.value.action === 'ok') router.push('/desktop') }
