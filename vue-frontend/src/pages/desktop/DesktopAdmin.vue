      ...
      <table class="w-full border-collapse text-sm table-fixed">
        ...
        <tbody>
          ...
          <tr v-for="(item, idx) in filtered" :key="item.id" class="border-t border-slate-100 hover:bg-blue-50 transition-all duration-150" :class="{'bg-blue-50': selected.includes(item.id)}">
            ...
            <td class="px-2 py-3 text-center text-slate-600">{{ item.category || '未分类' }}</td>
...
function handleAction(action, id) {
  if (action === 'view') router.push('/desktop/detail/' + id)
  else if (action === 'edit') router.push('/desktop/edit/' + id)
  else if (action === 'delete') {
    modal.value = { visible: true, message: '确定要删除这条记录吗？', type: 'confirm', action: 'delete-single', payload: id }
  }
}
...
async function doBatchDelete() {
  try {
    await apiDesktop.batchDelete([...selected.value])
    selected.value = []
    await loadData()
...
async function loadData() {
  try { const res = await apiDesktop.list(); items.value = res.data }
