  ...
  <MainLayout>
    ...
    <div class="cmd-grid" :class="{ 'grid-4': sidebarCollapsed }">
      <div v-for="item in filtered" :key="item.id" class="cmd-card" @click="handleClick(item)">
        <FavoriteButton class="card-favorite" module="desktop" :item="item" />
...
import { apiDesktop, formatDate, apiClicks } from '../../api/index.js'
...
const allCats = computed(() => {
  const keys = new Set()
  items.value.forEach(i => { if (i.category) keys.add(i.category) })
...
function handleClick(item) {
  recordView('desktop', item)
  apiClicks.record('desktop', item.id, item.title).catch(() => {})
  router.push('/desktop/detail/' + item.id)
}

onMounted(async () => {
  try {
    const res = await apiDesktop.list()
