<template>
  <MainLayout>
    ...
    <SearchBar v-model="search" placeholder="搜索网络命令..." :showCat="true"
      module="cmd-admin"
          ...
          <tr v-for="(item, idx) in filtered" :key="item.id" class="border-t border-slate-100 hover:bg-blue-50 transition-all duration-150" :class="{'bg-blue-50': selected.includes(item.id)}">
            ...
            <td class="px-2 py-3 text-center whitespace-nowrap">
              <span v-for="cfg in (item.configs||[]).slice(0, 3)" :key="cfg.vendor"
                ...
                :style="{ background: getVendorColor(cfg.vendor)+'15', color: getVendorColor(cfg.vendor), borderColor: getVendorColor(cfg.vendor)+'40' }">
                {{ getVendorName(cfg.vendor) }}
              </span>
            </td>
            <td class="px-2 py-3 text-center text-slate-600">{{ item.cat === getCatLabel(item.cat, CAT_MAP) ? item.cat : item.cat + ' - ' + getCatLabel(item.cat, CAT_MAP) }}</td>
...
function handleAction(action, id) {
  if (action === 'view') router.push('/cmd/detail/' + id)
  else if (action === 'edit') router.push('/cmd/edit/' + id)
  else if (action === 'delete') {
    modal.value = { visible: true, message: '确定要删除这条命令吗？', type: 'confirm', action: 'delete-single', payload: id }
  }
}

async function doDelete(id) {
  const result = await submitWithApproval('cmd', 'DELETE', null, id, () => apiTopics.delete(id))
...
async function doBatchDelete() {
  try {
    await apiTopics.batchDelete([...selected.value])
    selected.value = []
    await loadData()
...
async function loadData() {
  try { const res = await apiTopics.list(); items.value = res.data }
