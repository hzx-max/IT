<template>
  <MainLayout>
    ...
    <SearchBar v-model="search" placeholder="搜索网络命令..." :showVendor="true" :showCat="true"
      module="cmd"
      ...
      <table class="w-full border-collapse text-sm">
        ...
        <tbody>
          ...
          <tr v-for="(item, idx) in filtered" :key="item.id" class="border-t border-slate-100 hover:bg-blue-50 transition-all duration-150" :class="{'bg-blue-50': selected.includes(item.id)}">
            ...
            <td class="px-4 py-3 text-center text-slate-600">{{ item.cat === getCatLabel(item.cat, CAT_MAP) ? item.cat : item.cat + ' - ' + getCatLabel(item.cat, CAT_MAP) }}</td>
...
async function doDelete(id) {
  try { await apiTopics.delete(id); selected.value = selected.value.filter(i => i !== id); await loadData() }
