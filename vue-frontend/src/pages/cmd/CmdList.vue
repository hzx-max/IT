<template>
  <MainLayout>
    ...
    <SearchBar v-model="search" placeholder="搜索命令..." :showVendor="false" :showCat="true"
      module="cmd-list"
    ...
    <div class="cmd-grid" :class="{ 'grid-4': sidebarCollapsed }">
      <div v-for="item in filtered" :key="item.id" class="cmd-card" @click="handleClick(item)">
        <FavoriteButton class="card-favorite" module="cmd" :item="item" />
        <div class="cmd-card-body">
          <CardCarousel :topo="item.topo" />
          <h3>{{ item.title }}</h3>
...
.cmd-desc{font-size:17px;color:#64748b;line-height:1.5;margin:0 0 6px;text-indent:2em;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
